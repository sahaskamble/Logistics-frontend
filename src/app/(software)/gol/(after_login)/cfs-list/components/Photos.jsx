...
import { X, Camera, CameraIcon, Upload } from 'lucide-react';
...
const Photos = ({ 
  ...
  const getImageUrl = (filename) => {
    return `${PB_URL}/api/files/service_provider/${providerId}/${filename}`;
  };

  const mainImage = images[0] ? getImageUrl(images[0]) : null;
  const thumbnailImages = images.slice(1, 5).map(img => getImageUrl(img));
    ...
    <div className="border rounded-xl p-4 shadow-md shadow-foreground/40 relative overflow-hidden">
      <div className="bg-[#D4E6D2] rounded-lg p-3 mb-3 shadow-sm relative">
        ...
        <div className="absolute bottom-4 right-4 flex gap-2">
          {canEdit && (
            <label className="bg-blue-600 text-white px-3 py-2 rounded-lg shadow-lg flex items-center gap-2 text-sm sm:text-base cursor-pointer hover:bg-blue-700 transition-colors">
          ...
          <button
            onClick={() => setShowAllPhotos(true)}
            className="bg-green-800 text-white px-3 py-2 rounded-lg shadow-lg flex items-center gap-2 text-sm sm:text-base hover:bg-green-900 transition-colors"
          >
