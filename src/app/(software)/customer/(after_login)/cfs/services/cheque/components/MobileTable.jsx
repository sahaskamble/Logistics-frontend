...
export default function MobileTable() {
  ...
  const filteredRequests = cheques.filter(request => {
    const matchesSearch =
      request.id.toLowerCase().includes(searchQuery.toLowerCase()) ||
      request.invoiceNo.toLowerCase().includes(searchQuery.toLowerCase()) ||
  ...
  return (
    <div className="border rounded-xl flex flex-col p-4">
      <div className="flex-1 overflow-y-auto">
        <h2 className="text-xl font-semibold text-green-900 mb-4">Cheque Payments</h2>
        ...
        <div className="px-4 pb-4">
          {filteredRequests.map((request, index) => (
            <div key={index} className="bg-[var(--accent)] rounded-lg p-3 mb-3 shadow-sm">
              ...
              <p className="text-sm text-gray-600 mb-1">Order Id:  {request.order.id}</p>
              <p className="text-sm text-gray-600 mb-1">Consignee:  {request.consigneeName}</p>
              <p className="text-sm text-gray-600 mb-1">Invoice Id:  {request.invoiceId}</p>
              <p className="text-sm text-gray-600 mb-1">Uploaded On:  {request.date}</p>
              <p className="text-sm text-gray-600 mb-1">Bank Name:  {request.bankName}</p>
              <div className="text-sm text-gray-600 mb-1">Amount: Rs. {request.amount}</div>
              <p className="text-sm text-gray-600 mb-1">Cheque No:  {request.chequeNo}</p>
              <div className="text-sm text-gray-600 mb-1">Remarks: {request.remarks}</div>
