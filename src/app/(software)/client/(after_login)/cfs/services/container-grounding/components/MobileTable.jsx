...
export default function MobileTable() {
  ...
  const filteredRequests = containerGroundings.filter(request => {
    const matchesSearch =
  ...
  return (
    <div className="border rounded-xl flex flex-col p-4">
      <div className="flex-1 overflow-y-auto">
        <h2 className="text-xl font-semibold text-foreground mb-4">Groundings List</h2>
        ...
        <div className="px-4 pb-4">
          {filteredRequests.map((request, index) => (
            <div key={index} className="bg-[var(--accent)] rounded-lg p-3 mb-3 shadow-sm">
              ...
              <p className="text-sm text-gray-600 mb-1">Grounding Zone: {request.groundingZone}</p>
              <p className="text-sm text-gray-600 mb-1">Grounding Type: {request.groundingType}</p>
