...
export default function Table() {
  ...
  const columns = [
    ...
    },
    {
      ...
      filterable: true,
      cell: ({ row }) => <div>{row.original.equipmentType}</div>,
    },
    {
      id: 'remarks',
      accessorKey: 'remarks',
    ...
    },
    {
      ...
      cell: ({ row }) => (
        <div className='flex gap-2 items-center'>
          <Link
            href={`/client/cfs/services/special-equipments/view/${row.original.id}`}
          >
            <Eye
              size={18}
  ...
  return (
    <div className="border rounded-lg p-6 mb-4">
      <div className="flex justify-between items-center mb-4">
        <h2 className="text-xl font-semibold">Equipments Usage List</h2>
        <Form />
      </div>
      <DataTable columns={columns} data={specialEquipments} />
    </div>
  );
};
