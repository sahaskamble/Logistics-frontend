...
import { Search, Download, Eye, Pencil, TrashIcon, MoveRight } from 'lucide-react';
import Input from '@/components/ui/Input';
import { specialEquipments } from '@/constants/cfs/special-equipments';
import Form from './Form';

export default function MobileTable() {
  const [searchQuery, setSearchQuery] = useState('');

  const filteredRequests = specialEquipments.filter(request => {
    const matchesSearch =
  ...
  return (
    <div className="border rounded-xl flex flex-col p-4">
      <div className="flex-1 overflow-y-auto">
        <h2 className="text-xl font-semibold text-foreground mb-4">Equipments Usage</h2>
        ...
        <div className="px-4 pb-4">
          {filteredRequests.map((request, index) => (
            <div key={index} className="bg-[var(--accent)] rounded-lg p-3 mb-3 shadow-sm">
              ...
              <div className="text-sm text-gray-600 mb-1">Product Type: {request.productType}</div>
