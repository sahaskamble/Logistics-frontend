...
export default function MobileTable() {
  ...
  const filteredRequests = taxInvoices.filter(request => {
    const matchesSearch =
        ...
        <div className="px-4 pb-4">
          {filteredRequests.map((request, index) => (
            <div key={index} className="bg-[var(--accent)] rounded-lg p-3 mb-3 shadow-sm">
              ...
              <h1 className="font-medium"># {request.invoiceNo}</h1>
              ...
              <p className="text-sm text-gray-600 mb-1">CHA:  {request.chaName}</p>
              <p className="text-sm text-gray-600 mb-1">Uploaded On:  {request.to}</p>
              <p className="text-sm text-gray-600 mb-1">GSTIN:  {request.gstin}</p>
              <p className="text-sm text-gray-600 mb-1">Total Amount:  {request.grossTotal}</p>
              <p className="text-sm text-gray-600 mb-1">Note:  {request.remarks}</p>
